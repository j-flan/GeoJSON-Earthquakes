...
#The latitude, longitude, and magnitude are parsed from the object notation for each significant earthquake 
...
#This code embeds a google map using a google cloud API. The coordinates from the earthquake with the greatest #magnitude are input into the HTTP format and uses the place marker to mark the location on the interactive map. 
#The map is displayed using IFrame to a specific pixel size.
print("lat: ",maxlat,", long: ",maxlong, ", magnitude: ",maxmag,"\n")

api = "ENTER YOUR KEY HERE" #key for embeded usage
